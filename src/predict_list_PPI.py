...
def get_args():
    ...
    parser.add_argument(
        "-list_fn",
        required=True,
        help="List file containing input pMSA, L1 (length of the first protein)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000,
        help="Number of paired sequences to include for inference. Default is 5000",
    )
...
def prep_input(msa, L1, L2, device):
    ...
    qlen = L1 + L2
    xyz_t = (
        INIT_CRDS.reshape(1, 1, 27, 3).repeat(1, qlen, 1, 1)[None].to(device)
    )  # (B, T, L, 27, 3)
    t1d = torch.nn.functional.one_hot(
        torch.full((1, qlen), 20, device=device).long(), num_classes=21
    ).float()  # all gaps
    ...
    mask_t_2d = (
        mask_t_2d.float() * chain_idx.float()[:, None]
    ...
    for key in network_input:
        if key == "msa_orig":
            seq, msa_seed, msa_extra, mask_msa, msa_seed_gt = MSAFeaturize(
                network_input["msa_orig"], {"MAXLAT": 128, "MAXSEQ": 1024}, p_mask=-1.0
            )
    ...
    with torch.no_grad():
        ...
        for i_cycle in range(N_cycle):
            ...
            input_i = _get_model_input(
                network_input, output_i, return_raw=return_raw, use_checkpoint=False
            )
            ...
            prob = np.sum(
                prob_dist[0].permute(1, 2, 0).detach().cpu().numpy()[:L1, L1:, :20],
                axis=-1,
            ).astype(np.float16)
            ...
            lp.write(
                pair
                + "\t"
                + str(round(p_bind.item(), 6))
                + "\t"
                + str(round(endT - startT, 6))
                + "\n"
            )
