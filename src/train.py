...
def load_checkpoint(chk_fn, map_location):
    try:
        return torch.load(chk_fn, map_location=map_location, weights_only=False)
    except TypeError:
        return torch.load(chk_fn, map_location=map_location)
...
class Trainer:
    ...
    def load_model(
        ...
        checkpoint = load_checkpoint(chk_fn, map_location)
    ...
    def train_model(self, rank, world_size):
        ...
        valid_pdb_sampler = data.distributed.DistributedSampler(
            valid_pdb_set, num_replicas=world_size, rank=rank
        )
        valid_compl_sampler = data.distributed.DistributedSampler(
            valid_compl_set, num_replicas=world_size, rank=rank
        )
        ...
        valid_pdb_loader = data.DataLoader(
            valid_pdb_set, sampler=valid_pdb_sampler, **LOAD_PARAM
        )
        valid_compl_loader = data.DataLoader(
            valid_compl_set, sampler=valid_compl_sampler, **LOAD_PARAM
        )
        ...
        if rank == 0:
            print(
                "# of parameters:",
                sum(p.numel() for p in ddp_model.parameters() if p.requires_grad),
            )
        ...
        for epoch in range(loaded_epoch + 1, self.n_epoch):
            ...
            train_tot, train_loss = self.train_cycle(
                ddp_model,
                train_loader,
                optimizer,
                scheduler,
            ...
            valid_tot, valid_loss = self.valid_pdb_cycle(
                ddp_model, valid_pdb_loader, rank, gpu, world_size, epoch
            )
            F1_50 = self.valid_ppi_cycle(
                ddp_model,
                valid_compl_loader,
                valid_neg_loader,
                rank,
    ...
    def _prepare_input_common(self, inputs, gpu):
        ...
        t2d = xyz_to_t2d(
            xyz_t, mask_t_2d
        )  # 2D features for templates (B, T, L, L, 37+6+1)
    ...
    def _get_model_input(
        self, network_input, output_i, return_raw=False, use_checkpoint=False
    ):
        """
        ...
        for key in network_input:
            if key == "msa_orig":
                seq, msa_seed, msa_extra, mask_msa, msa_seed_gt = MSAFeaturize(
                    network_input["msa_orig"], self.loader_param
                )
    ...
    def train_cycle(
        ...
        for inputs in train_loader:
            network_input, true_crds, mask_crds, eval_PPI, negative = (
                self._prepare_input_common(inputs, gpu)
            )
            counter += 1
            ...
            for i_cycle in range(N_cycle):
                with ExitStack() as stack:
                    ...
                    input_i, mask_msa, msa_seed_gt = self._get_model_input(
                        network_input,
                        output_i,
                    ...
                    if i_cycle < N_cycle - 1:
                        continue
                    loss, loss_s = self._get_loss_and_misc(
                        output_i,
                        msa_seed_gt,
                        mask_msa,
                        true_crds,
            ...
            if counter % N_PRINT_TRAIN == 0:
                if rank == 0:
                    ...
                    sys.stdout.write(
                        "Local: [%04d/%04d] Batch: [%05d/%05d] Time: %8.1f | Loss: %8.4f | %s | Max mem %.4f\n"
                        % (
                            epoch,
                            self.n_epoch,
        ...
        if rank == 0:
            train_time = time.time() - start_time
            sys.stdout.write(
                "Train: [%04d/%04d] Time: %8.1f | Loss: %8.4f | %s\n"
                % (
                    epoch,
                    self.n_epoch,
                    train_time,
    ...
    def valid_pdb_cycle(self, ddp_model, valid_loader, rank, gpu, world_size, epoch):
        ...
        if rank == 0:
            train_time = time.time() - start_time
            sys.stdout.write(
                "Monomer: [%04d/%04d] Time: %8.1f | Loss: %8.4f | %s\n"
                % (
                    epoch,
                    self.n_epoch,
                    train_time,
    ...
    def valid_ppi_cycle(
            ...
            for inputs in valid_pos_loader:
                ...
                for i_cycle in range(N_cycle):
                    with ExitStack() as stack:
                        ...
                        if i_cycle < N_cycle - 1:
                            continue
                        loss, loss_s, p_bind = self._get_loss_and_misc(
                            output_i,
                            msa_seed_gt,
                            mask_msa,
                            true_crds,
        ...
        if rank == 0:
            train_time = time.time() - start_time
            sys.stdout.write(
                "Hetero: [%04d/%04d] Time: %8.1f | Loss: %8.4f | %s\n"
                % (
                    epoch,
                    self.n_epoch,
                    train_time,
        ...
        with torch.no_grad():  # no need to calculate gradient
            ...
            for inputs in valid_neg_loader:
                ...
                if valid_loss == None:
                    valid_loss = torch.zeros_like(loss_s.detach())
                valid_loss += loss_s.detach()
        valid_inter_50 = torch.tensor(
            [TP_50, FP_50, FN_50], device=valid_loss.device
        ).float()
        valid_tot /= float(counter * world_size)
        valid_loss /= float(counter * world_size)
        ...
        if rank == 0:
            ...
            train_time = time.time() - start_time
            sys.stdout.write(
                "PPI: [%04d/%04d] Time: %8.1f | Loss: %8.4f | %s | %.4f %.4f %.4f\n"
                % (
                    epoch,
                    self.n_epoch,
                    train_time,
